...
const Product = (props) => {
    ...
    const {id, product_name, product_image, ratings, numOfReviews, Origin_price, Promo_price } = props;
        ...
        <div className="flex flex-col items-center gap-2 px-2 py-6 relative">
            ...
            <Link to={`/product/${id}`} className="flex flex-col items-center text-center group">
                <div className="w-36 h-36">
                    <img draggable="false" className="w-full h-full object-contain" src={product_image} alt={product_name} />
                </div>
                <h2 className="text-sm mt-4 group-hover:text-primary-blue">{product_name.length > 50 ? `${name.substring(0, 50)}...` : name}</h2>
            </Link>
            {/* <!-- image & product title --> */}
            ...
            <div className="flex flex-col gap-2 items-center">
                ...
                <div className="flex items-center gap-1.5 text-md font-medium">
                    <span>TND {Promo_price}</span>
                    <span className="text-gray-500 line-through text-xs">TND {Origin_price}</span>
