        ...
        <>
            ...
            <main className="w-full mt-20">
                ...
                <div className="flex flex-col gap-2 items-center justify-center sm:w-4/6 sm:mt-4 m-auto mb-7 bg-white shadow rounded p-6 pb-12">
                    ...
                    <h1 className="text-2xl font-semibold"> {success ? "Félicitation" : "Votre commande à échouée !!"}</h1>
                    <p className="mt-4 text-lg text-gray-800">Redirection vers {success ? "products" : "cart"} dans {time} sec</p>
                    <Link to={success ? "/products" : "/cart"} className="bg-primary-blue mt-2 py-2.5 px-6 text-white uppercase shadow hover:shadow-lg rounded-sm">allez à {success ? "produits" : "panier"}</Link>
                </div>
                {/* <!-- row --> */}
