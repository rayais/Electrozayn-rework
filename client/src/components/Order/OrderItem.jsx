        ...
        <Link to={`/order_details/${orderId}`} className="flex p-4 items-start bg-white border rounded gap-2 sm:gap-0 hover:shadow-lg">
            ...
            <div className="flex flex-col sm:flex-row justify-between w-full">
                ...
                <div className="flex flex-col gap-1 overflow-hidden">
                    ...
                    <p className="text-xs text-gray-500 mt-2">Quantité: {quantity}</p>
                    <p className="text-xs text-gray-500">Totale: TND {(quantity * price).toLocaleString()}</p>
                </div>

                <div className="flex flex-col sm:flex-row mt-1 sm:mt-0 gap-2 sm:gap-20 sm:w-1/2">
                    <p className="text-sm">TND {price.toLocaleString()}</p>
                        ...
                        <p className="text-sm font-medium flex items-center gap-1">
                            {orderStatus === "Shipped" ? (
                                ...
                                <>
                                    ...
                                    Commandé(s) le {formatDate(createdAt)}
                                </>
                            )}
                        </p>
